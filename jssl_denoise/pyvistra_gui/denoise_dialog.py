...
Four tabs: Train (fits a new D-Net/N-Net pair via ``Trainer.fit``), Denoise
(runs a trained checkpoint over one source), Batch (the Denoise tab's
settings replayed across every checked file in a folder), and Monitor
(shared loss/mu_mse plot + run log, reused across all three).
...
class DenoiseDialog(QDialog):
    ...
    def _build_train_tab(self, layout, SourceSelector, RegionSelector, viewer):
        ...
        cfg_form.addRow("NLL beta:", self.nll_beta_spin)
    ...
    def _start_training(self):
        ...
        worker = TrainingWorker(frames, config)
    ...
    def _on_train_epoch_metrics(self, epoch, metrics):
        ...
        if mu_mse is not None:
            self.convergence_plot.append_point("mu_mse", mu_mse, axis="right")
        self._last_epoch_info["mu_mse"] = mu_mse
    ...
    def _checkpoint_summary(self) -> str:
        ...
        if info.get("mu_mse") is not None:
            parts.append(f"mu_mse={info['mu_mse']:.3f}")
        return "  ".join(parts)
            ...
            try:
                ...
                if reasons:
                    ...
                    self.batch_file_list.set_flag(
                        ...
                        "jssl_denoise is 2D/2D+time only.",
                    )
                else:
