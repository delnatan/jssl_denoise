...
for a ready-to-use ``Denoiser`` without caring which.
"""
...
class CheckpointSelector(QGroupBox):
    ...
    def build_denoiser(self, device: torch.device) -> Optional[Denoiser]:
        """Build a ready-to-use `Denoiser` for the current selection, or
        `None` if nothing usable is selected."""
    ...
    def _update_info(self) -> None:
        ...
        if data == self._SESSION and self._session_checkpoint is not None:
            cfg = self._session_checkpoint.get("config", {})
            self.info_label.setText(
                f"base_filters={cfg.get('base_filters', '?')}  tile_size={cfg.get('tile_size', '?')}"
            )
        elif data == self._LOAD_FILE and self._file_path:
            try:
                cfg = load_checkpoint(self._file_path).get("config", {})
                self.info_label.setText(
                    f"base_filters={cfg.get('base_filters', '?')}  tile_size={cfg.get('tile_size', '?')}"
                )
